...
function Booksession() {
  ...
  const [userData, setUserData] = useState(null);
  const [loading, setLoading] = useState(false);
  const [selecetdResult, setSelecetdResult] = useState(null);
  const navigate = useNavigate();
    ...
    if (selectedDate) {
      ...
      if (selected.getDay() === 0 || selected.getDay() === 6) {
        ...
        selecetSlot(selectedDate, time)
          .then((data) => {
            const formattedData = data?.remainingSlots.map((slot) => ({
              slotId: slot.slotId,
              slotDescription: slot.slotDescription,
            }));
  ...
  const validateForm = () => {
    ...
    if (!formData.counselingType) newErrors.counselingType = "Session type is required";
    if (!formData.date) newErrors.date = "Date is required";
    if (!formData.slot) newErrors.slot = "Slot is required";
    if (!formData.field) newErrors.field = "Type of counseling is required";
    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  };
  ...
  useEffect(() => {
    const interval = setInterval(() => setShine((prev) => !prev), 4000);
  ...
  <div
    className="relative px-10 py-5 text-base font-bold rounded-full shadow-xl"
    style={{
