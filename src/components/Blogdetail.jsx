...
//                   .filter(
//                     (b) =>
//                       blog?.type &&                   // Ensure current blog has a 'type'
//                       b?.type === blog?.type &&       // Match blogs of the same type
//                       b?.blogId !== blog?.blogId      // Exclude the current blog
//                   )
//                   .slice(0, 3)
//                   .map((b, index) => (
//                     <div
//                       key={index}
//                       className="flex flex-col sm:flex-row w-full sm:w-1/2 lg:w-full"
//                     >
//                       <img
//                         src={b.imagePath}
//                         alt={b.name}
...
//           <h3 className="text-2xl text-center lg:text-left px-4 sm:px-10 lg:px-0">
//             Testimonials
//           </h3>
...
//             "The guidance and support I received has been transformative. The
//             personalized approach helped me develop effective strategies for
//             managing my anxiety and stress."
//           </p>
...
//             "Working with my consultant has been life-changing. They provided me
//             with practical tools and insights that helped me overcome my
//             challenges."
//           </p>
...
const Blogdetail = () => {
  ...
  const location = useLocation();
  const { blogId, initialStatus } = location.state || {};
  const navigate = useNavigate();
  ...
  useEffect(() => {
    ...
    getBlogdetails()
      .then((res) => {
        setBlogDetails(res || []);
        if (blogId && res?.length) {
          const found = res.find((b) => b.blogId === blogId || b.id === blogId);
          setBlog(found);
          setStatus(initialStatus || found?.like || 0);
        }
      })
  ...
  const handleAction = async (blogId, status) => {
    try {
      const userString = localStorage.getItem("userData");
      if (!userString) {
        message.error("Please login to react to blogs.");
        return;
      }
      const user = JSON.parse(userString);
      const userId = user?.userId || user?.id;
    ...
    <div>
      ...
      <div className="flex flex-col items-center justify-center pt-14 px-4">
        ...
        <motion.p
          ...
          animate={{ opacity: 1 }}
          transition={{ duration: 1, delay: 0.5 }}
          className="text-base sm:text-lg text-center text-gray-700 max-w-2xl"
        >
      ...
      {/* Blog Content */}
      <div className="max-w-screen-xl mx-auto px-4 pt-20 sm:px-8">
        <img
          src={blog?.imagePath}
          alt={blog?.name}
          className="w-full h-96 object-cover rounded-lg"
        ...
        <div className="flex flex-col md:flex-row gap-8 mt-12">
          <div className="md:w-2/3">
            <h1 className="text-3xl font-semibold text-black">{blog?.name}</h1>
  ...
  >
    ...
    <span>Dislike</span>
          ...
          <div className="md:w-1/3">
            <h2 className="text-2xl font-semibold text-gray-800 mb-4">Recommended Blogs</h2>

            <div className="flex flex-col gap-6">
              {blogDetails
                .filter((b) => b?.type === blog?.type && b?.blogId !== blog?.blogId)
                .slice(0, 3)
                .map((b) => (
                  <div
                    key={b.blogId}
      ...
      <div className="px-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 py-10 max-w-[2000px] mx-auto">
          {[...blogDetails]
            ...
            .slice(0, 3)
            .map((b) => (
              <div key={b.blogId} className="bg-white rounded-lg border shadow p-6">
                <img
                  src={b.imagePath}
                  alt="Blog Post"
                  className="w-full h-48 object-cover rounded-lg mb-4"
                />
                <h2 className="text-2xl font-bold text-black mb-2">{b.name}</h2>
      ...
      <div className="max-w-screen-xl mx-auto py-20 px-5">
        <div className="text-center">
          <h3 className="text-2xl font-semibold text-gray-700 mb-4">Testimonials</h3>
          <h1 className="text-4xl font-bold text-black">
            Healing Words - Testimonials from our clients
          </h1>
        </div>
        {/* You can add Testimonials component here */}
      </div>
    </div>
  );
};
